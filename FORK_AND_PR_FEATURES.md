...
| --- | --- | --- |
| `adam01110/fifc` | Exact PR `#49`, exact PR `#52`, exact PR `#54`, and exact PR `#60`; `#49` already includes the PR `#61` behavior | Current fork state: custom rm command, custom fzf opts, preserved directory search opts, eza-first directory preview, fixed escaped query handling, and no forced extra Tab bind |
...
| --- | --- | --- | --- | --- |
| `adam01110/fifc` | 4 | `#49`, `#52`, `#54`, `#60` | `#61` equivalent via `#49` behavior | `#36` |
...
| --- | --- | --- | --- | --- |
| Reapply bindings when `fish_key_bindings` changes | - | Y | - | - |
| Fix path display when current path contains spaces | - | Y | - | - |
...
3. Cherry-pick the extra `justbispo/fifc` fixes worth considering beyond the PRs:
   - `e70150d` binding persistence
   - `caf694e` paths-with-spaces display fix
...
- `adam01110/fifc` now carries exact PR `#49`, `#52`, `#54`, and `#60` behavior in one fork.

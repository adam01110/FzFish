function _fifc_reload_depth -d "Reload file/directory listing at specific depth"
    ...
    if type -q fd
        ...
        set -l fd_base_opts $fifc_fd_opts $type_opt --max-depth $depth --color=always --no-ignore $hidden_flag $fd_custom_opts
        ...
        if set -q fifc_show_hidden; and test "$fifc_show_hidden" = true
            find . $escaped_path -maxdepth $depth $fifc_find_opts $find_type_opt \

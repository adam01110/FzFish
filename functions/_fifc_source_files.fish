function _fifc_source_files -d "Return a command to recursively find files"
    ...
    if type -q fd
        ...
        set -l fd_base_opts $fifc_fd_opts --max-depth 1 --color=always --no-ignore $hidden_flag $fd_custom_opts
        ...
        echo "find . $escaped_path -maxdepth 1 $fifc_find_opts ! -path . -print 2>/dev/null | sed 's|^\./||'"
    else
        echo "find . $escaped_path -maxdepth 1 $fifc_find_opts ! -path . ! -path '*/.*' -print 2>/dev/null | sed 's|^\./||'"
    end
end
